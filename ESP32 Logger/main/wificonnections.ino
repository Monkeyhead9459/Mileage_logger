...
std::map<String, String> dateLineMap;
...
void wifiupload(){
  ...
  String failedLines = "";
  int batchCount = 0;
  std::vector<String> successfulLines;
  ...
  while (file.available()) {
    ...
    successfulLines.push_back(line);//
    ...
    if (batchCount == 25 || file.available() == 0) {
      ...
      if (httpCode == 200) {
        ...
        for (String l : successfulLines) {
          // Extract date from each line again to be safe
          int idx = l.indexOf(',');
          if (idx > 0) {
            String thisDate = l.substring(0, idx);
            dateLineMap[thisDate] += l + "\n";
          }
        }
        // Write each date’s lines to the correct file
        for (auto const& entry : dateLineMap) {
          String backupFile = "/backup/" + entry.first + ".csv";
          File bkp = SD.open(backupFile, FILE_APPEND);
          if (bkp) {
            bkp.print(entry.second);  // write all lines for this date
            bkp.close();
          } else {
            Serial.println("Failed to write to file: " + backupFile);
          }
        }
        dateLineMap.clear();
        successfulLines.clear();

      } else {
        Serial.printf("Batch upload failed! Code: %d\n", httpCode);
        for (String l : successfulLines) {
          failedLines += l + "\n";  // only save if batch failed
        }
        successfulLines.clear();
      }
...
void performOTA(const char* url) {
  WiFiClientSecure client;
  client.setCACert(gitCert);
   client.setTimeout(10000);  // ⏱️ Set 10 second timeout

  HTTPClient https;
  Serial.println("Connecting to GitHub for update...");
...
bool checkForUpdate(String& newFirmwareURL) {
  WiFiClientSecure client;
  //client.setCACert(gitCert);
  client.setInsecure();  // ⚠️ Only use for testing — skips TLS check!
  client.setTimeout(10000);  // ⏱️ Set 10 second timeout
  ...
  if (!http.begin(client, versionUrl)) {
    Serial.println("Failed to connect to version.json URL.");
    return false;
  }
  ...
  if (httpCode == 200) {
    ...
    String latest = doc["latest"].as<String>();
    newFirmwareURL = doc["firmware"].as<String>();

    if (latest != CURRENT_VERSION) {
      Serial.println("New version available: " + latest);
      http.end();
      return true;
    } else {
